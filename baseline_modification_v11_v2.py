...
class XLSTMWrapper:
    def __init__(self, input_size, seq_len=1, hidden_size=64, num_layers=2, num_symbols=1, dropout=0.0, batch_size=32, lr=0.0001, epochs=100, patience=5, device='cuda'):
        self.device = torch.device(device if torch.cuda.is_available() else 'cpu')
        self.model = sLSTM(input_size=input_size, seq_len=seq_len, hidden_size=hidden_size, num_layers=num_layers, num_symbols=num_symbols).to(self.device)
...
# ----------------- 模型字典 -----------------
model_dict = {
    'xlstm': XLSTMWrapper(input_size=len(FEATURE_NAMES), seq_len=20, hidden_size=64, num_layers=2, num_symbols=num_symbols, batch_size=32, lr=0.001, epochs=100, patience=5, device='cuda'),
