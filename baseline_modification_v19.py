...
class CNNEncoderBase(nn.Module):
    ...
    def forward(self, x):
        ...
        y = y.permute(0,2,1) # [N,T,output_dim]
        return y
...
class KRNNEncoderBase(nn.Module):
    ...
    def forward(self, x):
        ...
        for rnn in self.rnn_modules:
            h, _ = rnn(x)  # [T,N,output_dim]
            hids.append(h)
        hids = torch.stack(hids, dim=-1) # [T,N,output_dim,dup_num]
        hids = hids.view(T, N, self.output_dim, self.dup_num)
        hids = hids.mean(dim=3) # [T,N,output_dim]
        hids = hids.permute(1,0,2) # [N,T,output_dim]
        return hids
...
class SandwichWrapper:
    def __init__(self,
        ...
        self.model = SandwichModel(
            fea_dim=self.input_dim,
            cnn_dim_1=cnn_dim_1,
            cnn_dim_2=cnn_dim_2,
            cnn_kernel_size=cnn_kernel_size,
            rnn_dim_1=rnn_dim_1,
    ...
    def fit(self, X_train, y_train, symbol_id_train, sample_weight=None, X_valid=None, y_valid=None, symbol_id_valid=None):
        # X_train: [N, T, F], Sandwich需要输入[N,1,T,F]
        ...
        for epoch in tqdm(range(self.epochs), desc='Sandwich Training Epochs'):
            epoch_loss = 0.0
            self.model.train()
            ...
            print(f"Sandwich Epoch {epoch + 1}/{self.epochs}, Training Loss: {epoch_loss:.4f}")
        ...
        if X_valid is not None and y_valid is not None:
            ...
            torch.save(best_param, os.path.join(MODEL_DIR, 'sandwich_best_param.pth'))
    ...
    def predict(self, X_test, symbol_id_test):
        ...
        X_test_4d = X_test[:, np.newaxis, :, :]  # [N,1,T,F]

        self.model.eval()
        with torch.no_grad():
            X_test_tensor = torch.tensor(X_test_4d, dtype=torch.float32).to(self.device)
