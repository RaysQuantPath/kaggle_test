...
class TransformerModel(nn.Module):
    def __init__(self, d_feat=79, d_model=64, nhead=4, num_layers=2, dropout=0.5, num_symbols=1):
        super(TransformerModel, self).__init__()
        self.feature_layer = nn.Linear(d_feat, d_model)
        self.pos_encoder = PositionalEncoding(d_model)
        encoder_layers = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dropout=dropout)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_layers=num_layers)
        self.decoder_layer = nn.Linear(d_model, num_symbols)
    ...
    def forward(self, src):
        ...
        output = self.decoder_layer(output.transpose(1, 0)[:, -1, :])  # [N, num_symbols]
        return output  # 返回所有 symbol 的预测

class TransformerWrapper:
    def __init__(self, input_size, seq_len=1, d_model=64, nhead=4, num_layers=2, dropout=0.5, batch_size=32, lr=0.0001, epochs=100, patience=5, device='cuda', num_symbols=1):
        self.device = torch.device(device if torch.cuda.is_available() else 'cpu')
        self.model = TransformerModel(d_feat=input_size, d_model=d_model, nhead=nhead, num_layers=num_layers, dropout=dropout, num_symbols=num_symbols).to(self.device)
