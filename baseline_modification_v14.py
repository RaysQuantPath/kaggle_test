...
class SFM_Model(nn.Module):
    def __init__(
        ...
        self.W_p = nn.Parameter(torch.empty(self.hidden_dim, self.output_dim))
    ...
    def init_states(self, x):
        init_state_h = torch.zeros(x.size(0), self.hidden_dim).to(self.device)
        init_state_p = torch.zeros(x.size(0), self.output_dim).to(self.device)

        init_state = torch.zeros(x.size(0), self.hidden_dim).to(self.device)
        init_freq = torch.zeros(x.size(0), self.freq_dim).to(self.device)
