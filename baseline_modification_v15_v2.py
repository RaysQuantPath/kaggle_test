...
class ALSTM_Model(nn.Module):
    def __init__(self, d_feat=79, hidden_size=64, num_layers=2, dropout=0.0, device='cuda'):
        ...
        self.att_fc_in = nn.Linear(in_features=self.hid_size, out_features=self.hid_size // 2)
        self.att_dropout = nn.Dropout(self.dropout)
        self.att_act = nn.Tanh()
        self.att_fc_out = nn.Linear(in_features=self.hid_size // 2, out_features=1, bias=False)
        self.to(self.device)
    ...
    def forward(self, x):
        ...
        att_hidden = self.att_fc_in(rnn_out)      # [batch, seq_len, hid_size/2]
        att_hidden = self.att_dropout(att_hidden)
        att_hidden = self.att_act(att_hidden)
        att_score = self.att_fc_out(att_hidden)   # [batch, seq_len, 1]
        att_score = torch.softmax(att_score, dim=1)
        out_att = torch.sum(rnn_out * att_score, dim=1) # [batch, hid_size]
        out = self.fc_out(torch.cat((rnn_out[:, -1, :], out_att), dim=1))  # [batch, 1]
        return out.squeeze(-1)

class ALSTMWrapper:
    def __init__(self, input_dim, num_symbols, hidden_dim=64, num_layers=2, dropout=0.0, batch_size=1024, lr=0.001, epochs=100, patience=5, device='cuda'):
        self.model = ALSTM_Model(d_feat=input_dim, hidden_size=hidden_dim, num_layers=num_layers, dropout=dropout, device=device)
        self.device = torch.device(device if torch.cuda.is_available() else 'cpu')
    ...
    def fit(self, X_train, y_train, symbol_id_train, sample_weight=None, X_valid=None, y_valid=None, symbol_id_valid=None):
        ...
        for epoch in tqdm(range(self.epochs), desc='ALSTM Training Epochs'):
            ...
            print(f"ALSTM Epoch {epoch + 1}/{self.epochs}, Training Loss: {epoch_loss:.4f}")
            ...
            if X_valid is not None and y_valid is not None:
                ...
                print(f"ALSTM Epoch {epoch + 1}/{self.epochs}, Validation Loss: {valid_loss:.4f}")
        ...
        if X_valid is not None and y_valid is not None:
            ...
            torch.save(best_param, os.path.join(MODEL_DIR, 'alstm_best_param.pth'))
