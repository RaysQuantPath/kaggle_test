...
def train(model_dict, model_name='lgb'):
    if TRAINING:
        ...
        groups = train_df['symbol_id'].reset_index(drop=True)

        # Initialize GroupKFold
        gkf = GroupKFold(n_splits=N_FOLD)

        for i, (train_index, valid_index) in enumerate(gkf.split(X, y, groups)):
            # Ensure that the training indices do not contain data from the validation set
            if max(train_index) > min(valid_index):
                print(f"Fold {i}: Overlapping indices detected. Skipping this fold to prevent data leakage.")
                continue
...
model_dict = {
    'lgb': lgb.LGBMRegressor(
        n_estimators=500, device='gpu', gpu_use_dp=True, objective='l2'
    ),
