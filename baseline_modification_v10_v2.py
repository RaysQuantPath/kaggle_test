...
class LNNWrapper:
    def __init__(self, input_dim, num_symbols, hidden_dim=64, batch_size=32, lr=0.001, epochs=100, patience=5,
                 device="cuda"):
        self.model = LTCRNN(input_dim=input_dim, hidden_dim=hidden_dim, num_symbols=num_symbols, device=device).to(
            device)
        ...
        self.loss_fn = nn.MSELoss(reduction='none')  # 使用 'none' 以应用样本权重
        ...
        for epoch in range(self.epochs):
            ...
            if X_valid is not None and y_valid is not None:
                ...
                with torch.no_grad():
                    ...
                    valid_predictions_selected = valid_predictions.gather(1, symbol_valid_tensor.unsqueeze(1)).squeeze(
                        1)
                    valid_loss = nn.functional.mse_loss(valid_predictions_selected, y_valid_tensor,
                                                        reduction='mean').item()
    ...
    def fit(self, X_train, y_train, symbol_id_train, X_valid=None, y_valid=None, symbol_id_valid=None,
        ...
        for epoch in range(self.epochs):
            ...
            for X_batch, y_batch, w_batch, symbol_id_batch in dataloader:
                ...
                predictions = self.model(X_batch)  # (batch_size, num_symbols)
                # 选择对应 symbol_id 的预测
                predictions_selected = predictions.gather(1, symbol_id_batch.unsqueeze(1)).squeeze(1)  # (batch_size)
    ...
    def fit(self, X_train, y_train, symbol_id_train, X_valid=None, y_valid=None, symbol_id_valid=None,
        ...
        for epoch in range(self.epochs):
            ...
            for X_batch, y_batch, w_batch, symbol_id_batch in dataloader:
                ...
                # 计算加权损失
                loss = self.loss_fn(predictions, y_batch)  # (batch_size)
                weighted_loss = (loss * w_batch).mean()  # 应用样本权重
            ...
            if X_valid_reshaped is not None and y_valid is not None:
                ...
                with torch.no_grad():
                    ...
                    valid_loss = nn.functional.mse_loss(valid_predictions, y_valid_tensor, reduction='mean').item()
