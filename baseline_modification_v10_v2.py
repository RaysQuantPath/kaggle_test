...
class LTCRNN(torch.nn.Module):
    ...
    def forward(self, x):
        _, hidden = self.rnn(x)
        output = self.fc(hidden[-1])  # 输出维度 [batch_size, num_symbols]
        return output

class LNNWrapper:
    def __init__(self, input_dim, num_symbols, hidden_dim=64, batch_size=32, lr=0.01, epochs=100, patience=5, device="cuda"):
    ...
    def fit(self, X_train, y_train, symbol_id_train, sample_weight=None, X_valid=None, y_valid=None, symbol_id_valid=None):
        ...
        for epoch in range(self.epochs):
            ...
            for X_batch, y_batch, symbol_batch, w_batch in dataloader:
                ...
                # 选择对应 symbol_id 的预测
                pred_selected = predictions.gather(1, symbol_batch.unsqueeze(1)).squeeze(1)

                # Compute weighted loss
                loss = self.loss_fn(pred_selected, y_batch)
            ...
            if X_valid is not None and y_valid is not None:
                ...
                with torch.no_grad():
                    ...
                    pred_selected = valid_predictions.gather(1, symbol_valid_tensor.unsqueeze(1)).squeeze(1)
                    valid_loss = torch.nn.functional.mse_loss(pred_selected, y_valid_tensor, reduction='mean').item()
    ...
    def predict(self, X_test, symbol_id_test):
        ...
        symbol_id_test = torch.tensor(symbol_id_test, dtype=torch.long).to(self.device)
        with torch.no_grad():
            predictions = self.model(X_test).squeeze(-1)
            pred_selected = predictions.gather(1, symbol_id_test.unsqueeze(1)).squeeze(1)
            return pred_selected.cpu().numpy()
    ...
    for i in range(N_FOLD):
        if TRAINING:
            ...
            if model_name == 'lgb':
                ...
                model.fit(X_train, y_train, symbol_id_train, sample_weight=w_train, X_valid=X_valid, y_valid=y_valid, symbol_id_valid=symbol_id_valid)
...
# ----------------- 模型字典 -----------------
model_dict = {
    'lnn': LNNWrapper(input_dim=len(FEATURE_NAMES), num_symbols=num_symbols, hidden_dim=64, batch_size=32, lr=0.001, epochs=100),
