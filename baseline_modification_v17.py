...
class GRUWrapper:
    def __init__(self, input_dim, num_symbols, hidden_dim=64, num_layers=2, dropout=0.0, batch_size=1024, lr=0.001, epochs=100, patience=5, device='cuda'):
        self.model = GRU_Model(d_feat=input_dim, hidden_size=hidden_dim, num_layers=num_layers, dropout=dropout, device=device)
        self.device = torch.device(device if torch.cuda.is_available() else 'cpu')
    ...
    def fit(self, X_train, y_train, symbol_id_train, sample_weight=None, X_valid=None, y_valid=None, symbol_id_valid=None):
        ...
        for epoch in tqdm(range(self.epochs), desc='GRU Training Epochs'):
            ...
            print(f"GRU Epoch {epoch + 1}/{self.epochs}, Training Loss: {epoch_loss:.4f}")
            ...
            if X_valid is not None and y_valid is not None:
                ...
                print(f"GRU Epoch {epoch + 1}/{self.epochs}, Validation Loss: {valid_loss:.4f}")
        ...
        if X_valid is not None and y_valid is not None:
            ...
            torch.save(best_param, os.path.join(MODEL_DIR, 'gru_best_param.pth'))
