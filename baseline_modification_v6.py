...
def train(model_dict, model_name='lgb'):
    if TRAINING:
        ...
        for i, (train_index, valid_index) in enumerate(outer_cv.split(X)):
            ...
            w_train_outer = w.iloc[train_index]
            X_valid_outer = X.iloc[valid_index]
            y_valid_outer = y.iloc[valid_index]
            w_valid_outer = w.iloc[valid_index]
            ...
            grid_search.fit(X_train_outer, y_train_outer, sample_weight=w_train_outer)

            best_model = grid_search.best_estimator_

            # 使用最佳模型进行验证
            y_pred = best_model.predict(X_valid_outer)
            r2_score = weighted_r2_score(y_valid_outer, y_pred, w_valid_outer)
            print(f"Fold {i}, R² Score: {r2_score:.4f}")

            joblib.dump(best_model, os.path.join(MODEL_DIR, f'{model_name}_{i}.model'))
            del X_train_outer, y_train_outer, w_train_outer
    else:
